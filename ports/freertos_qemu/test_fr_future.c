 ...
 * async_socket_* / async_listener_* (future_socket.c) —— 证 pal_socket/lwip_raw.c
...
task_t* task_arg(server_task) {
    ...
    gen_var(fut) = async_socket_accept(gen_var(listener));
    gen_yield(gen_var(fut));
    gen_var(conn) = (async_socket_t*)future_result(gen_var(fut));
    if (!gen_var(conn)) { printf("accept failed\n"); gen_return(1); }

    gen_var(fut) = async_socket_recv(gen_var(conn), gen_var(buf), sizeof(gen_var(buf)));
    gen_yield(gen_var(fut));
    gen_var(n) = (int)anet_code_of(future_result(gen_var(fut)));
    if (gen_var(n) <= 0) { printf("server recv fail n=%d\n", gen_var(n)); gen_return(1); }

    gen_var(fut) = async_socket_send(gen_var(conn), gen_var(buf), gen_var(n));
    gen_yield(gen_var(fut));
...
task_t* task_arg(client_task) {
    ...
    gen_var(fut) = async_socket_send(gen_var(sock), MSG, strlen(MSG));
    gen_yield(gen_var(fut));
    if (anet_code_of(future_result(gen_var(fut))) < 0) { printf("client send fail\n"); gen_return(1); }

    gen_var(fut) = async_socket_recv(gen_var(sock), gen_var(buf), sizeof(gen_var(buf)));
    gen_yield(gen_var(fut));
    gen_var(n) = (int)anet_code_of(future_result(gen_var(fut)));
...
int run_echo_loop(void) {   /* 复用 boot_raw.c 入口名 */
    ...
    async_listener_t *listener = async_listener_create(lsock);
