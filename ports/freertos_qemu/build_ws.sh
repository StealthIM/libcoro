#!/usr/bin/env bash
# 阶段 2b: 裸机 raw + asyncweb WebSocket server (明文 ws) 到 QEMU mps2-an385。
# 用法: build_ws.sh <out.elf>
#
# WS 握手需要 SHA1 (Sec-WebSocket-Accept) + rand (Sec-WebSocket-Key), 由
# wolfssl.c 的 crypto 原语 (anet_tls_sha1/rand_bytes) 提供 —— 即使明文 ws 也
# 要链 wolfSSL crypto。故基于 build_tls_raw (含 wolfSSL 组 D), 组 E 加 ws.c +
# stream.c + bare_tls_shim (补 file-based/sync_ssl 缺口)。
set -e
...
# 组 E: asyncweb WS server (ws.c) + stream + 真 wolfssl.c (crypto 原语) +
#        future_socket + pal_socket/lwip_raw + common + shim + 测试
OBJS+=" $(compile "$INC_AW" \
  "$AW/src/ws.c" "$AW/src/sock/stream.c" \
  "$AW/src/tls/wolfssl.c" "$AW/src/sock/future_socket.c" \
  "$AW/src/sock/pal_socket/lwip_raw.c" "$AW/src/common.c" \
  "$QD/bare_tls_shim.c" "$QD/test_fr_ws.c")"
