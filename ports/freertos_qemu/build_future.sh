...
#   E. asyncweb future_socket + pal_socket/lwip_raw + common + 测试
#      (asyncweb 私有头 + libcoro 私有头 + raw_arch + lwIP)
set -e

OUT="${1:-/tmp/fr_future.elf}"
OBJ=$(mktemp -d)
trap 'rm -rf "$OBJ"' EXIT
...
# 组 E: asyncweb future_socket + pal_socket/lwip_raw + common + 测试
OBJS+=" $(compile "$INC_AW" \
  "$AW/src/sock/future_socket.c" "$AW/src/sock/pal_socket/lwip_raw.c" \
  "$AW/src/common.c" "$QD/test_fr_future.c")"
